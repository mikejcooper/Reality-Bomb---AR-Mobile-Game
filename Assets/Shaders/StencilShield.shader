Shader "Stencil/Shield"
{

    Properties
    {
        _Color("_Color", Color) = (0.0,1.0,0.0,1.0)
        _Highlight("_Highlight", Color) = (0.0,1.0,0.0,1.0)
        ...
        _Rate("_Rate", Range(1.0,100.0)) = 20.0
    }
...
#pragma surface surf BlinnPhongEditor alpha vertex:vert
        ...
        void surf(Input IN, inout EditorSurfaceOutput o)
        {
            ...
            float4 UV_Pan0 = float4((IN.uv_Texture.xyxy).x,(IN.uv_Texture.xyxy).y + Multiply5.x,(IN.uv_Texture.xyxy).z,(IN.uv_Texture.xyxy).w); //Movement along texture
            float4 Multiply1 = UV_Pan0 * _Tile.xxxx;
            float4 Tex2D0 = tex2D(_Texture,Multiply1.xy);
            float4 Multiply2 = Tex2D0 * _Strength.xxxx;
            float4 Multiply0 = Pow0 * Multiply2; //Apply fresnel effect
            float4 Multiply3 = Clamp0 * Multiply0;
            float Offset = sin((dNorm/10 + -_Time)*_Rate) > 0.9 ? 1 : 0;
            o.Emission = Multiply3.xyz * ((1-Offset) * _Color.rgb + Offset * _Highlight.rgb);
